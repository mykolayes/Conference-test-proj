...
public class ConferenceApi {
    ...
    public Profile saveProfile(ProfileForm userProfile, User user) throws UnauthorizedException {
...
/**
    ...
    public Conference createConference(final User user, final ConferenceForm conferenceForm)
        ...
        final Key<Conference> conferenceKey = ofy().factory().allocateId(profileKey,Conference.class);
        		//Key.create(profileKey, Conference.class, id)
        ...
        queue.add(TaskOptions.Builder.withUrl("/sendMail").param("email", user.getEmail()).param("conferenceInfo", conference.toString())); //.param("key", key)
        //queue.add(TaskOptions.Builder.withURL(URL_for_task). param(param1).param(param2);
    ...
    public List<Conference> getConferencesCreated(final User user) throws UnauthorizedException, NotFoundException {
        ...
        String userId = user.getUserId();
        //Profile profile = getProfileFromUser(user);
        //Key parentKey = profile.get
        Key<Profile> parentKey = Key.create(Profile.class,userId);
        Query query = ofy().load().type(Conference.class).ancestor(parentKey).order("name");
        return query.list();
        */
            ...
            Profile profile = getProfileFromUser(user); //null; Change this;
            if (profile == null) {
            ...
            Key<Profile> parentKey = Key.create(Profile.class,userId);
            Query query = ofy().load().type(Conference.class).ancestor(parentKey).order("name");
            //List<String> keyStringsToAttend = query.list();
            List<Conference> createdConferences = query.list();
            return createdConferences;  //null; change this
        }
    //}
 ...
 /**
  ...
  /**
    ...
    public WrappedBoolean registerForConference(final User user,
    	...
    	//registerForConference_SKELETON
        // If not signed in, throw a 401 error.
        if (user == null) {
            throw new UnauthorizedException("Authorization required");
        }
 ...
 /**
    ...
    public Collection<Conference> getConferencesToAttend(final User user)
        ...
        List<String> keyStringsToAttend = profile.getConferenceKeysToAttend(); //null; change this

        // TODO
        // Iterate over keyStringsToAttend,
        // and return a Collection of the
        // Conference entities that the user has registered to attend
        Collection<Conference> registeredToConferences = new ArrayList(keyStringsToAttend.size());
        for (String s : keyStringsToAttend) {
        	registeredToConferences.add(getConference(s));
        }

        return registeredToConferences;  //null; change this
    }
    ...
    */   
    ...
    @Named("websafeConferenceKey") final String websafeConferenceKey
        ...
        WrappedBoolean result = ofy().transact(new Work <WrappedBoolean>() {
        	public WrappedBoolean run () { 
                ...
                if (!profile.getConferenceKeysToAttend().contains(
                        websafeConferenceKey)) {
                    return new WrappedBoolean (false, "Did not register");
                    /*
                } else if (conference.getSeatsAvailable() <= 0) {
                    return new WrappedBoolean (false, "No seats available");
                    */
                } else {
                    // All looks good, go ahead and free the seat
                    
                    // TODO
                    // Remove the websafeConferenceKey from the profile's
                    // conferencesToAttend property
                    profile.removeConferenceKeyToAttend(websafeConferenceKey);
        ...
        if (!result.getResult()) {
            ...
            else if (result.getReason() == "Did not register") {
                throw new ConflictException("You have not registered for this conference yet to free the seat");
            }
            /*
            else if (result.getReason() == "No seats available") {
                throw new ConflictException("There are no seats available");
    ...
    @ApiMethod(
    	    ...
    	    //TODO GET announcement from memcache by key and if it exists return it
